...
export async function fetchPagesByTypeAndSlug(
  params: {
    type?: string;
    slug?: string | null;
    locale: string;
...
export async function fetchPageAndMapToMeta(
  arg: Parameters<typeof fetchPagesByTypeAndSlug>[0]
) {
  ...
  if (response.data?.[0]?.attributes.seo)
    ...
    console.warn(
      `Unprovided seo details for ${arg?.type || "page-standart"} ${arg?.slug}`
    )
